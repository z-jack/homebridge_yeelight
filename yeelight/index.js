...
YeePlatform.prototype = {
	...
	onDevFound: function (dev) {
		...
		if (!found) {
			...
			if (dev.model == "desklamp") {
				lightbulbService.addCharacteristic(Characteristic.ColorTemperature).on('set', function (value, callback) {
					that.exeCmd(dev.did, "temperature", value, callback);
				}).value = dev.sat;
			}
		} else {
			lightbulbService
